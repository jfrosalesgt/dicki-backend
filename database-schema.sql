...
GO
CREATE PROCEDURE sp_Indicio_FindAll @codigo_caso NVARCHAR(50) = NULL, @id_fiscalia INT = NULL, @estado_actual NVARCHAR(50) = NULL, @activo BIT = NULL AS BEGIN SET NOCOUNT ON; SELECT i.id_indicio, i.codigo_indicio, i.descripcion_corta, i.estado_actual, i.fecha_hora_recoleccion, t.nombre_tipo AS tipo_indicio, inv.codigo_caso, inv.nombre_caso FROM Indicio i INNER JOIN TipoIndicio t ON i.id_tipo_indicio = t.id_tipo_indicio INNER JOIN Escena e ON i.id_escena = e.id_escena INNER JOIN Investigacion inv ON e.id_investigacion = inv.id_investigacion WHERE i.activo = ISNULL(@activo, i.activo) AND (@codigo_caso IS NULL OR inv.codigo_caso LIKE '%' + @codigo_caso + '%') AND (@id_fiscalia IS NULL OR inv.id_fiscalia = @id_fiscalia) AND (@estado_actual IS NULL OR i.estado_actual = @estado_actual) ORDER BY i.fecha_hora_recoleccion DESC; END
GO
CREATE PROCEDURE sp_Indicio_Update @id_indicio INT, @descripcion_corta NVARCHAR(255) = NULL, @ubicacion_especifica NVARCHAR(100) = NULL, @activo BIT = NULL, @usuario_actualizacion NVARCHAR(50) AS BEGIN SET NOCOUNT ON; UPDATE Indicio SET descripcion_corta = ISNULL(@descripcion_corta, descripcion_corta), ubicacion_especifica = ISNULL(@ubicacion_especifica, ubicacion_especifica), activo = ISNULL(@activo, activo), usuario_actualizacion = @usuario_actualizacion, fecha_actualizacion = GETDATE() WHERE id_indicio = @id_indicio; END
GO
